...
class SLTInstall{
	private $url_slt_zip = "http://eugene-sukhodolskiy.github.io/server-logic-trail/slt.zip";

	public function get_slt_in_zip(){
		$zip = file_get_contents($this -> url_slt_zip);
		file_put_contents('slt.zip', $zip);
	}

	public function unpackage_slt(){
		$slt_zip = new ZipArchive;
		if($slt_zip -> open('slt.zip') === true){
			$slt_zip -> extractTo('./');
			$slt_zip -> close();
		}else{
			echo "<h3>ERROR UNPACKAGE!</h3>";
		}
	}

	public function configuration_slt(){
		file_put_contents('slt/config/db.config.php', db_config_template($_POST));
		$app_name = $_POST['project_name'];
		$index_file = file_get_contents('index.php');
		$index_file = str_replace("'app_name' => '_APP_'", "'app_name' => '{$app_name}'", $index_file);
		file_put_contents('index.php', $index_file);
		rename('_APP_', $app_name);
	}
	...
	public function remove_slt_zip_and_self(){
		unlink('slt.zip');
		unlink('install.php');
	}
	...
	public function install_start(){
		...
		$this -> get_slt_in_zip();
		$this -> unpackage_slt();
		$this -> configuration_slt();
		$this -> remove_slt_zip_and_self();
		header('Location: /');
	}
...
function db_config_template($config){
	return "<?php
...
function config_page_template(){
	...
	<head>
		<meta charset="UTF-8">
		<title>SLT Configuration</title>
		<?php config_css() ?>
	</head>
	<body>
		<div class="container config">
			<h2>SLT Configuration</h2>
			<?php show_config_form() ?>
		</div>	
	</body>
	</html>
...
function show_config_form(){
	...
	<form method="post">
		...
		<input type="text" id="project_name" name="project_name" placeholder="Project name" value="MyApp">
		<br>
		<h5>DataBase</h5>
		...
		<input type="text" id="user" name="user" placeholder="User" value="root">
		<br>
		<label for="db_name">DB Name</label>
		<input type="text" id="db_name" name="db_name" placeholder="DB Name">
		<br>
